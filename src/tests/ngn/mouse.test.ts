...
class MockWheelEvent {
  constructor(public type: string, public config: { deltaY?: number }) {}
  get deltaY() {
...
export default testSuite(async ({ describe }) => {
  describe('mouse', () => {
    ...
    test('should handle mouse down and up events', () => {
      ...
      onMouseDown(new MockMouseEvent('mousedown', { button: Number(MouseButton.Mouse1) }) as any);
      mouseUpdate();
      expect(m.mouse.getButton('Shoot')).toEqual({
        pressed: true,
        justPressed: true,
      ...
      onMouseUp(new MockMouseEvent('mouseup', { button: Number(MouseButton.Mouse1) }) as any);
      mouseUpdate();
      expect(m.mouse.getButton('Shoot')).toEqual({
        pressed: false,
        justPressed: false,
    ...
    test('should handle mouse move events', () => {
      onMouseMove(new MockMouseEvent('mousemove', { movementX: 100, movementY: 50 }) as any);
      const m = mouse();
