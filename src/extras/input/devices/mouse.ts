...
import { MouseMapping, StandardMouse, MouseButton } from "./mappings/mouse";
...
export const mouseUpdate = (): void => {
  for (const [button, value] of Object.entries(observedMouseState.buttons)) {
    mouseState.buttons[button] = {
      ...value,
      justReleased:
        !value.pressed && buttonsDownLastFrame.buttons?.[button]?.pressed,
    };
...
export const mouse = () => ({
  mouse: {
    ...
    getButton(b: string): ButtonState {
      ...
      // Get from mapping.
      const button = Object.keys(mouseMapping.buttons)[
        Object.values(mouseMapping.buttons).indexOf(b)
      ];
    ...
    getAxis(a: string): number {
      ...
      const ax = Object.keys(mouseMapping.axes)[
        Object.values(mouseMapping.axes).indexOf(a)
      ];
...
const drawAccel = () => {
  document.getElementById(
    "accel"
  ).innerHTML = `${mouseState.axes[0]}, ${mouseState.axes[1]}`;
};
